...
qg.substitutions['M(A)']        = "- 1 + integ(integ(A - integ(A,'z'),'y'),'x')/Axy"

# equations 2.29(a-c) + 3.2 [Sprague, 2006]
qg.add_equation("dt(tf) - L(tf) = - J(si,tf) + L(fi)*M(fi*tf)")

qg.add_equation("dz(L(fi)) + D(si) = 0",condition="(nx != 0) and (ny != 0)")
qg.add_equation("fi = 0",condition="(nx == 0) or (ny == 0)")

qg.add_equation("dz(si) - Ra*tf - D(fi) = 0",condition="(nx != 0) and (ny != 0)")
